data { // 
  ...
  int N_species; // Number of species in mock data
  int N_species_all; // Number of species in environmental data
  ...
  int sample_data[N_obs_samp,N_species_all]; 
  ...
  // 1 indicates to modify that species, 0 indicates to do the normal way 
  vector[N_species_all] max_alpha; // indicator of species that we will assume are equivalent to max species.
  vector[N_species_all] min_alpha; // indicator of species that we will assume are equivalent to min species.
  vector[N_species_all] zero_alpha; // indicator of species that we will assume are equivalent to ref species.
...
parameters {
  ...
  vector<lower=0>[N_species-1] tau; // single overdispersion sd for multinomial, changed from N-species-all
  vector[N_species_all-1] alpha_raw;
  vector[N_b_samp_col] beta_raw[N_species_all-1];
  // vector[N_obs_samp] eta_samp_raw[N_species-1];
  vector[N_obs_mock] eta_mock_raw[N_species-1]; //changed from N_species_all
...
transformed parameters {
  ...
  vector[N_species_all] alpha; // coefficients
  vector[N_b_samp_col] beta[N_species_all]; // coefficients
  ...
  matrix[N_obs_samp,N_species_all] mu_samp; // estimates, in log space 
  matrix[N_obs_mock,N_species] mu_mock; // estimates, in log space 
  
  { // local variables declaration
  matrix[N_obs_samp,N_species_all] logit_val_samp; 
  matrix[N_obs_mock,N_species] logit_val_mock; 
  matrix[N_species_all,N_obs_samp] prob_samp_t; 
  matrix[N_species,N_obs_mock] prob_mock_t;
    
    
    // Fixed effects components.
    alpha[1:(N_species_all-1)] = (alpha_prior[1] + alpha_raw * alpha_prior[2]); // non-centered param
    // This section allows designated species to be either zero, the maximum alpha or the minimum alpha.
    alpha[N_species_all] = 0; // final species is zero (reference species)
    alpha = alpha .* zero_alpha +
                  max_alpha * max( alpha .* zero_alpha) + 
                  min_alpha * min( alpha .* zero_alpha) ; // non-centered param
    alpha[N_species_all] = 0; // final species is zero (reference species)
    
    beta[N_species_all] = rep_vector(0.0,N_b_samp_col);// final species is zero (reference species)
    for (l in 1:(N_species_all-1)){
        beta[l] = beta_prior[1] + beta_raw[l] * beta_prior[2];
    }

    // over dispersion
    //tau = rep_vector(tau_base,N_species-1);
    eta_mock[N_species] = rep_vector(0.0,N_obs_mock); // final species is zero (reference species) EDITED THIS TOO!!
...
model {
  ...
  for(i in 1:(N_species_all-1)){ //changed from _all
    // eta_samp_raw[i] ~ std_normal(); // N(0,tau)
    eta_mock_raw[i] ~ std_normal(); // N(0,tau)
  }
for (i in 1:(N_species_all - 1)){
...
generated quantities {
  matrix[N_obs_samp_small,N_species_all] int_samp_small; // estimates, in proportion space
  
    { // local variables delcaration
  matrix[N_obs_samp_small,N_species_all] logit_val_samp_ppd;
  matrix[N_species_all,N_obs_samp_small] prob_samp_t_ppd;
