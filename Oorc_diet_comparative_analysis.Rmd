...
author: "Amy M. Van Cise^1,2\\*†^, M. Bradley Hanson^3^, Candice Emmons^3^, Dan Olsen^4^, Craig O. Matkin^4^, Abigail Wells^5^, Kim M. Parsons^3^"
...
#load("Oorc_95tax_newref_halibut_phyloseq_majortaxfilt_prey_hist_minor_species.Rdata") # from Oorc_prey_histogram.R
```
...
Once amplification bias corrections were applied to all samples, prey species were only included in downstream analyses if they represented >1% of the reads in 4 or more samples in the dataset, in order to avoid potential bias from genotyping error or secondary prey (i.e. species eaten by killer whale prey). Species that occur at >1% of the diet in 1-3 samples are considered "minor prey species" and are described in the results, but are not included in downstream analyses because it is unknown whether these represent opportunistic prey or secondary prey (prey eaten by killer whale target prey).
...
Multivariate analysis of variance in diet composition, including only major prey species, indicated significant variance in diet between populations (PERMANOVA R^2^ = `r round(pop_difference["Population","R2"]*100, digits = 1)`%, p-value = `r pop_difference["Population", "Pr(>F)"]`) and among months (R^2^ = `r round(pop_difference["month","R2"]*100, digits = 1)`%, p-value = `r pop_difference["month", "Pr(>F)"]`) and years (R^2^ = `r round(pop_difference["year","R2"]*100, digits = 1)`%, p-value = `r pop_difference["year", "Pr(>F)"]`).  
...
## FUNDING {-#FUNDING}
This work was funded by a research grant awarded to K.M. Parsons and the North Gulf Oceanic Society by the National Fish and Wildlife Federation with funding from Shell and SeaWorld. Additional funding to support SRKW sample collection and processing was provided by the National Oceanic and Atmospheric Administration. NEED FUNDING FOR NGOS SAMPLING HERE. 

## DATA ACCESSIBILITY {-#DATA-ACCESSIBILITY}
All raw sequence data are archived in NCBI Genbank (Accession #XXXXX). All R scripts used to generate this paper are available on GitHub (https://github.com/UW-WADE-lab/NPac-RKW-diet-comparison).
